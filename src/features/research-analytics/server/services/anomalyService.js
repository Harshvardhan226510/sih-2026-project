...
import { mean, sum, stdDev, calculateAnomaly, calculateAnomalyPercentage, calculateZScore } from '../utils/statistics.js';
export class AnomalyService {
    ...
    async getAnomalyAnalytics(location, targetStartDate, targetEndDate, baselineStartDate, baselineEndDate, metric = 'rainfall') {
        ...
        const { records: baselineRecords, provenance } = await this.adapter.fetchHistoricalRecords(location, baseStart, baseEnd);
        // Fetch target period records
        const { records: targetRecords } = await this.adapter.fetchHistoricalRecords(location, targetStartDate, targetEndDate);
        ...
        baselineRecords.forEach(r => {
            ...
            if (!baselineDailyMap.has(mmdd))
                baselineDailyMap.set(mmdd, []);
