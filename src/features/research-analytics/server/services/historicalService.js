...
import { mean, min as minVal, max as maxVal, median, stdDev, percentile, sum, movingAverage } from '../utils/statistics.js';
export class HistoricalService {
    ...
    async getHistoricalAnalytics(location, startDate, endDate, metric = 'rainfall', aggregation = 'monthly') {
        ...
        for (let i = 0; i < aggregatedPoints.length; i++) {
            if (ma7[i] !== null)
                aggregatedPoints[i].rollingAvg7d = ma7[i];
            if (ma30[i] !== null)
                aggregatedPoints[i].rollingAvg30d = ma30[i];
            if (ma90[i] !== null)
                aggregatedPoints[i].rollingAvg90d = ma90[i];
        }
