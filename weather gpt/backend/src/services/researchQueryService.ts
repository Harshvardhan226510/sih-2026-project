...
export class ResearchQueryService {
  ...
  private extremeService: ExtremeEventService;
  ...
  private parseIntent(query: string, preferredLocation?: string) {
    const q = query.toLowerCase();

    // Extract metric
    let metric: WeatherMetric = 'rainfall';
    ...
    if (q.includes('monsoon') || q.includes('rainy')) season = 'Monsoon';
    else if (q.includes('summer')) season = 'Summer';
    else if (q.includes('winter')) season = 'Winter';
    else if (q.includes('post-monsoon') || q.includes('autumn')) season = 'Post-Monsoon';

    // Extract analysis type
    let type: 'HISTORICAL' | 'TREND' | 'ANOMALY' | 'COMPARISON' | 'EXTREME' | 'CLIMATE_PROFILE' | 'FORECAST_ACCURACY' = 'HISTORICAL';
    ...
    let aggregation: AggregationPeriod = 'yearly';
    if (q.includes('month')) aggregation = 'monthly';
    else if (q.includes('day') || q.includes('daily')) aggregation = 'daily';
    else if (q.includes('week')) aggregation = 'weekly';
  ...
  async processResearchQuery(request: ResearchQueryRequest): Promise<ResearchQueryResponse> {
    const parsed = this.parseIntent(request.query, request.preferredLocation);
    const locA = resolveLocation(parsed.locations[0]);

    let analyticsData: any = null;
    let chartType: 'line' | 'bar' | 'area' | 'comparison-line' | 'timeline' | 'radar' = 'line';
    let explanation = '';
    ...
    if (parsed.type === 'COMPARISON') {
      ...
      const compRes = await this.comparisonService.compareLocations(
        locA,
        locB,
      ...
      const trendRes = await this.trendService.getTrendAnalytics(
        locA,
        parsed.dateRange.start,
        parsed.dateRange.end,
        parsed.metric
      ...
      const anomRes = await this.anomalyService.getAnomalyAnalytics(
        locA,
        parsed.dateRange.start,
        parsed.dateRange.end,
        undefined,
      ...
      const extRes = await this.extremeService.detectExtremeEvents(
        locA,
        parsed.dateRange.start,
        parsed.dateRange.end
      );
      ...
      // Historical
      const histRes = await this.historicalService.getHistoricalAnalytics(
        locA,
        parsed.dateRange.start,
        parsed.dateRange.end,
        parsed.metric,
